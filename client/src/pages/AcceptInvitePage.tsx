/**
 * AcceptInvitePage — ERP Integrated Business Simulator
 * Student self-registration via teacher-generated invite link.
 * Route: /invite/:token
 */
...
import { Layers, Eye, EyeOff, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
...
export default function AcceptInvitePage() {
  ...
  const { data: inviteInfo, isLoading: loadingInvite, error: inviteError } = trpc.students.getInviteInfo.useQuery(
    { token: token ?? '' },
    { enabled: !!token, retry: false }
  );

  const acceptInvite = trpc.students.acceptInvite.useMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await acceptInvite.mutateAsync({ token: token ?? '', name, password });
      setSuccess(true);
    } catch (err: unknown) {
  ...
  const labelStyle = { color: 'oklch(0.75 0.01 255)', fontSize: '0.875rem', fontWeight: 500, display: 'block', marginBottom: '0.375rem' };
      ...
      <div className="relative w-full max-w-md mx-4">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm mt-1" style={{ color: 'oklch(0.55 0.01 255)' }}>
            {isFr ? 'Collège de la Concorde' : 'Collège de la Concorde'}
          </p>
        </div>
        ...
        <div style={cardStyle}>
          ...
          {!loadingInvite && inviteError && (
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              <p style={{ color: 'oklch(0.55 0.01 255)', fontSize: '0.875rem' }}>
                {isFr ? 'Ce lien d\'invitation n\'est plus valide. Contactez votre professeur.' : 'This invitation link is no longer valid. Contact your professor.'}
              </p>
            </div>
          )}
          ...
          {success && (
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              <p style={{ color: 'oklch(0.55 0.01 255)', fontSize: '0.875rem' }}>
                {isFr ? `Bienvenue, ${name} ! Vous pouvez maintenant vous connecter.` : `Welcome, ${name}! You can now log in.`}
              </p>
              <button
          ...
          {/* Registration form */}
          {!loadingInvite && inviteInfo && !success && (
            <>
              <div className="mb-5">
                <h2 className="text-lg font-bold" style={{ color: 'oklch(0.92 0.005 255)', fontFamily: 'Space Grotesk, sans-serif' }}>
                  {isFr ? 'Créer votre compte' : 'Create your account'}
                </h2>
                <p className="text-sm mt-1" style={{ color: 'oklch(0.55 0.01 255)' }}>
                  {isFr ? `Invitation pour : ` : `Invitation for: `}
                  <span style={{ color: 'oklch(0.70 0.18 255)' }}>{inviteInfo.email}</span>
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {/* Full name */}
                <div>
                  <label style={labelStyle}>{isFr ? 'Nom complet' : 'Full name'}</label>
                  <input
                    type="text"
                    value={name}
                ...
                {/* Password */}
                <div>
                  <label style={labelStyle}>{isFr ? 'Mot de passe' : 'Password'}</label>
                  <div className="relative">
                    <input
                      ...
                      onBlur={e => (e.target.style.borderColor = 'oklch(0.25 0.02 255)')}
                    />
                    <button type="button" onClick={() => setShowPassword(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 p-1" style={{ color: 'oklch(0.50 0.01 255)' }}>
        ...
        <p className="text-center text-xs mt-4" style={{ color: 'oklch(0.35 0.01 255)' }}>
          {isFr ? 'Collège de la Concorde · ERP-SIM · Prof. Thiago Gibran' : 'Collège de la Concorde · ERP-SIM · Prof. Thiago Gibran'}
        </p>
      </div>
    </div>
  );
