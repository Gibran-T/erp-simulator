    ...
    <FioriShell title="Tableau de Bord — Enseignant" breadcrumbs={[{ label: "Tableau de bord" }]}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {cards.map((card) => (
          <button key={card.label} onClick={() => navigate(card.href)}
            className="bg-white border border-[#d9d9d9] rounded-md p-4 text-left hover:border-[#0070f2] hover:shadow-sm transition-all group">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        ...
        <Card className="border-[#d9d9d9]">
          <CardHeader className="pb-2 px-5 pt-4">
            <CardTitle className="text-xs font-semibold text-[#0f2a44] flex items-center gap-2">
          ...
          <CardContent className="px-5 pb-4">
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center">
                ...
                <p className="text-[10px] text-gray-500">Simulations</p>
              </div>
              <div className="text-center">
              ...
              <>
                <div className="grid grid-cols-3 gap-2">
                  <div className="text-center">
                    ...
                    <p className="text-[10px] text-gray-500">Simul.</p>
                  </div>
                  <div className="text-center">
      ...
      <div className="bg-white border border-[#d9d9d9] rounded-md">
        ...
        <div className="divide-y divide-[#ededed]">
          {recentRuns.length === 0 && (
            <div className="py-10 text-center text-gray-400 text-xs">Aucune simulation en cours</div>
          )}
          {recentRuns.map((run: any) => {
            ...
            return (
              <div key={run.run?.id ?? run.runId} className={`px-5 py-3 flex items-center justify-between ${isDemo ? "bg-[#fafafa]" : ""}`}>
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className="text-xs font-semibold text-[#0f2a44]">{run.user?.name ?? `Étudiant #${run.run?.userId}`}</p>
                    {isDemo ? (
                  ...
                  <p className="text-[10px] text-gray-400">{run.scenario?.name} · {run.completedSteps?.length ?? 0} étapes</p>
                </div>
      ...
      <div className="mt-4 p-4 rounded-md border-2 border-[#5b4b8a] bg-gradient-to-r from-[#ede7f6] to-[#f3e5f5] flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-[#5b4b8a] rounded-md flex items-center justify-center shrink-0">
            <MonitorPlay size={20} className="text-white" />
          </div>
          <div>
            <p className="text-sm font-bold text-[#3d1f6e]">Mode Démonstration — Simulateur ERP/WMS</p>
            <p className="text-xs text-[#5b4b8a] mt-0.5">Lancez le simulateur directement pour vos démonstrations en classe. Le score pédagogique est affiché en temps réel (non officiel).</p>
          </div>
        </div>
        <button
      ...
      <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-3">
        <div className="p-4 rounded-md border border-blue-200 bg-blue-50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Layers size={16} className="text-blue-600" />
            <div>
              <p className="text-xs font-semibold text-blue-900">Module 2 — Exécution d'entrepôt et gestion des emplacements</p>
              <p className="text-[10px] text-blue-700">Rangement · Capacité d'emplacement · FIFO · Précision inventaire</p>
            </div>
          </div>
          <button onClick={() => navigate("/student/module2")} className="text-xs text-blue-600 hover:underline font-medium">Accéder →</button>
        </div>
        <div className="p-4 rounded-md border border-emerald-200 bg-emerald-50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <TrendingUp size={16} className="text-emerald-600" />
            <div>
              <p className="text-xs font-semibold text-emerald-900">Module 3 — Contrôle des stocks et réapprovisionnement</p>
              <p className="text-[10px] text-emerald-700">Inventaire cyclique · Écarts · Ajustements · Min/Max · Stock de sécurité</p>
            </div>
          </div>
        ...
        <div className="p-4 rounded-md border border-blue-200 bg-blue-50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold text-blue-900">Module 4 — Indicateurs de performance logistique</p>
              <p className="text-[10px] text-blue-700">Rotation · Taux de service · Taux d’erreur · Lead time · Diagnostic KPI</p>
            </div>
          </div>
          <button onClick={() => navigate("/student/module4")} className="text-xs text-[#0070f2] hover:underline font-medium">Accéder →</button>
        </div>
        <div className="p-4 rounded-md border border-purple-200 bg-purple-50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <FileText size={16} className="text-[#7b1fa2]" />
            <div>
              <p className="text-xs font-semibold text-purple-900">Module 5 — Simulation opérationnelle intégrée</p>
              <p className="text-[10px] text-purple-700">Réception · Rangement FIFO · Inventaire · Réapprovisionnement · KPI · Décision</p>
            </div>
          </div>
