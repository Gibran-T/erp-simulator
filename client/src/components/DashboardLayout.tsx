...
const NAV_ITEMS: NavItem[] = [
  ...
  { label: 'Modules ERP', href: '/modules', icon: <BookOpen size={18} /> },
  { label: 'Simulateur', href: '/simulator', icon: <Play size={18} /> },
  { label: 'Cohortes', href: '/cohorts', icon: <Users size={18} />, roles: ['admin', 'teacher'] },
  { label: 'Travaux', href: '/assignments', icon: <ClipboardList size={18} /> },
  { label: 'Suivi', href: '/monitoring', icon: <BarChart3 size={18} />, roles: ['admin', 'teacher'] },
  { label: 'Guide Professeur', href: '/teacher-guide', icon: <BookMarked size={18} />, roles: ['admin', 'teacher'] },
  { label: 'Administration', href: '/admin', icon: <Settings size={18} />, roles: ['admin'] },
];
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'oklch(0.10 0.015 255)' }}>
      {/* Mobile overlay */}
      {sidebarOpen && (
        <div
      ...
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-4 py-5" style={{ borderBottom: '1px solid oklch(1 0 0 / 6%)' }}>
          <img src={LOGO_ICON} alt="ERP Logo" className="w-9 h-9 rounded-lg object-cover" />
          <div>
            <div className="text-sm font-bold leading-tight" style={{ fontFamily: 'Space Grotesk', color: 'oklch(0.93 0.005 255)' }}>
              ERP Simulator
            </div>
        ...
        {/* User info */}
        <div className="px-4 py-3 mx-3 mt-3 rounded-lg" style={{ background: 'oklch(0.16 0.020 255)' }}>
            ...
            return (
              <Link key={item.href} href={item.href}>
                <div
                  ...
                  onClick={() => setSidebarOpen(false)}
                >
                  <span style={{ color: isActive ? 'oklch(0.80 0.16 255)' : 'oklch(0.55 0.010 255)' }}>
                    {item.icon}
                  </span>
                ...
                {ERP_MODULES.map(mod => (
                  <Link key={mod.id} href={`/modules/${mod.id}`}>
                    <div
                      ...
                      style={{
                        color: location === `/modules/${mod.id}` ? MODULE_COLORS[mod.id] : 'oklch(0.55 0.010 255)',
        ...
        <div className="px-3 pb-2">
          <div className="rounded-lg px-3 py-2 text-xs" style={{ background: 'oklch(0.60 0.20 255 / 10%)', border: '1px solid oklch(0.60 0.20 255 / 20%)' }}>
            <div className="font-semibold" style={{ color: 'oklch(0.75 0.16 255)' }}>Programme 2 — ERP</div>
            <div style={{ color: 'oklch(0.50 0.010 255)' }}>30 heures · 5 modules</div>
          </div>
        </div>

        {/* Logout */}
        <div className="px-3 pb-4" style={{ borderTop: '1px solid oklch(1 0 0 / 6%)', paddingTop: '12px' }}>
          <button
            onClick={logout}
            ...
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = 'oklch(0.55 0.010 255)'; (e.currentTarget as HTMLElement).style.background = 'transparent'; }}
          >
            <LogOut size={16} />
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex items-center gap-4 px-6 py-4 shrink-0"
          style={{ borderBottom: '1px solid oklch(1 0 0 / 6%)', background: 'oklch(0.11 0.016 255)' }}>
          <button
            className="lg:hidden"
